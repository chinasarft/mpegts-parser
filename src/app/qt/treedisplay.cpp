...
static void addTreeViewOfSPS_profile_idc (QStandardItem *root, AVD::ProfileIdcExt* p) {
...
void ShowTreeViewOfAVCDecoderConfigurationRecord(QStandardItem *root, AVD::AVCDecoderConfigurationRecord* r) {
    ...
    for (int i = 0; i < r->numOfSequenceParameterSets; i++) {
        row.clear();
        auto spsret = AVD::ParseSps(r->sps[i].sequenceParameterSetNALUnit+1, r->sps[i].sequenceParameterSetLength);
        if (spsret.second == 0) {
...
void ShowTreeViewOfAudioSpecificConfig(QStandardItem *root, AVD::AudioSpecificConfig* r) {
    ...
    std::vector<uint32_t> vs = {
        r->audioObjectType, (uint32_t)(r->GetSamplingFrequency()), r->channelConfiguration
    };
